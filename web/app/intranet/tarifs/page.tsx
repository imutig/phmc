        ...
        <div className="py-4 md:p-8">
            ...
            >
                <div>
                    <div className="flex items-center gap-3 mb-2">
                        <DollarSign className="w-6 md:w-8 h-6 md:h-8 text-red-500" />
                ...
                {canEdit && (
                    <button
                        ...
                        className="flex items-center gap-2 px-3 md:px-4 py-2 bg-red-600 hover:bg-red-500 text-white font-display font-bold text-xs md:text-sm uppercase tracking-widest transition-colors self-start"
                    >
            ...
            <div className="relative mb-4 md:mb-6">
                ...
                <input
                    ...
                    className="w-full bg-[#141414] border border-[#2a2a2a] pl-10 md:pl-12 pr-4 py-2.5 md:py-3 text-white text-sm md:text-base focus:border-red-500/50 transition-colors"
                />
            </div>
                    ...
                    {filteredCategories.length === 0 ? (
                        ...
                        filteredCategories.map(category => (
                            ...
                            >
                                {/* Category Header */}
                                <div
                                    className="flex items-center justify-between p-3 md:p-4 cursor-pointer hover:bg-[#1a1a1a]"
                                    onClick={() => toggleCategory(category.id)}
                                >
                                    <div className="flex items-center gap-2 md:gap-3">
                                        {expandedCategories.has(category.id) ? (
                                            <ChevronUp className="w-4 md:w-5 h-4 md:h-5 text-gray-500" />
                                        ) : (
                                            <ChevronDown className="w-4 md:w-5 h-4 md:h-5 text-gray-500" />
                                        )}
                                        <h3 className="font-display font-bold text-sm md:text-lg uppercase">{category.name}</h3>
                                        <span className="text-[10px] md:text-xs text-gray-500">({category.care_types?.length || 0})</span>
                                    </div>
                                    {canEdit && (
                                        <div className="flex items-center gap-1 md:gap-2" onClick={e => e.stopPropagation()}>
                                            ...
                                            <button
                                                ...
                                                className="p-1.5 md:p-2 text-gray-400 hover:bg-white/10 rounded transition-colors hidden sm:block"
                                            >
                                    ...
                                    {expandedCategories.has(category.id) && (
                                        ...
                                        >
                                            {category.care_types?.length === 0 ? (
                                                <p className="p-3 md:p-4 text-gray-500 text-sm italic">Aucun soin dans cette catégorie</p>
                                            ) : (
                                                <div className="divide-y divide-[#2a2a2a]">
                                                    {[...(category.care_types || [])].sort((a, b) => {
                                                        ...
                                                        <div key={care.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 md:p-4 hover:bg-[#1a1a1a] gap-2">
                                                            <div className="flex items-start sm:items-center gap-2 md:gap-3 flex-1 min-w-0">
                                                                <button
                                                                    onClick={() => toggleFavorite(care.id)}
                                                                    className={`p-1 rounded transition-colors flex-shrink-0 ${favorites.has(care.id)
                                                                        ? 'text-yellow-400 hover:text-yellow-300'
                                                                        : 'text-gray-600 hover:text-gray-400'
                                                                        }`}
                                                                    title={favorites.has(care.id) ? 'Retirer des favoris' : 'Ajouter aux favoris'}
                                                                >
                                                                    <Star className={`w-3.5 md:w-4 h-3.5 md:h-4 ${favorites.has(care.id) ? 'fill-current' : ''}`} />
                                                                </button>
                                                                <div className="min-w-0 flex-1">
                                                                    <p className="font-sans font-medium text-sm md:text-base truncate">{care.name}</p>
                                                                    {care.description && (
                                                                        <p className="text-xs md:text-sm text-gray-500 truncate">{care.description}</p>
                                                                    )}
                                                                </div>
                                                            </div>
                                                            <div className="flex items-center gap-2 md:gap-4 ml-7 sm:ml-0">
                                                                <span className="font-mono text-red-400 font-bold text-sm md:text-base">${care.price}</span>
                                                                <button
                                                                    onClick={() => addToCart(care)}
                                                                    className="px-2 md:px-3 py-1 bg-red-600/20 text-red-400 text-xs md:text-sm font-bold hover:bg-red-600/30 transition-colors whitespace-nowrap"
                                                                >
                                                                    + Ajouter
                                                                </button>
                                                                {canEdit && (
